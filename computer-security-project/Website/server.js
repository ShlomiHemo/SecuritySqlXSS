...
const xss = require('xss'); // Add this package
...
app.post('/api/clients', (req, res) => {
  ...
  if (!sanitizedData.first_name || !sanitizedData.last_name || !sanitizedData.phone_number || !sanitizedData.email) {
    return res.status(400).json({ message: 'Missing required fields' });
  }

  const emailCheckQuery = 'SELECT * FROM clients WHERE email = ?';
  db.query(emailCheckQuery, [sanitizedData.email], (err, results) => {
    if (err) {
      return res.status(500).json({ message: 'Database error' });
    }
    ...
    const clientData = { first_name: sanitizedData.first_name, last_name: sanitizedData.last_name, phone_number: sanitizedData.phone_number, address: sanitizedData.address, email: sanitizedData.email, package: sanitizedData.package };
